...
export default async function guess(
  ...
  if (session?.messageId === interaction.message.id && session.expiresAt > Date.now() && responses.length < 10) {
    ...
    const { id: emoji } = await fetch(
      `https://wsrv.nl/?url=https://cdn.discordapp.com/avatars/${interaction.user.id}/${interaction.user.avatar}&w=128&mask=heart&tint=${Math.random() > 0.5 ? "blue" : "orange"}&bg=${Math.random() > 0.5 ? "blue" : "orange"}&encoding=base64`,
    ).then(async (r) =>
